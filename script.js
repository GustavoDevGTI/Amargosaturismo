...
function initAttractionCardToggles() {
    document.querySelectorAll('#pontos .attraction-card').forEach((card, index) => {
        const content = card.querySelector('.card-content');
        const summaryParagraph = content?.querySelector('p:not(.subtitle-card):not(.highlights)');
        let button = content?.querySelector('.attraction-card__toggle');

        if (!content || !summaryParagraph) {
            return;
        }

        const paragraphId = summaryParagraph.id || `ponto-card-texto-${index + 1}`;
        const fullText = summaryParagraph.dataset.fullText || summaryParagraph.innerHTML.trim();
        const summaryText = summaryParagraph.dataset.summaryText || fullText;

        summaryParagraph.id = paragraphId;
        summaryParagraph.dataset.fullText = fullText;
        summaryParagraph.innerHTML = summaryText;
        if (!button) {
            button = document.createElement('button');
            button.type = 'button';
            button.className = 'attraction-card__toggle';
            button.textContent = 'Ver mais';
            content.appendChild(button);
        }

        button.setAttribute('aria-expanded', 'false');
        button.setAttribute('aria-controls', paragraphId);

        button.addEventListener('click', () => {
            const isExpanded = card.classList.toggle('is-expanded');

            summaryParagraph.innerHTML = isExpanded ? fullText : summaryText;
